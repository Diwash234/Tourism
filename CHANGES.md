...
surfaces from Round 18) — sidebar, navbar, direction, distance, navigation —
and for a way to see **the distance to all ~6,000 destinations** from a source
or the current location, with direction/turn info and the places marked on a
**real Nepal map** so Rolpa, Humla, Jumla, Darchula, Dadeldhura, Rukum Chhota,
etc. are easy to find.

### 1. A real UI icon set for the whole app (original artwork)
- **53 new vector icons** in `public/icons/ui/`, generated by
  `scripts/generate-ui-icons.mjs` (`node scripts/generate-ui-icons.mjs`):
  - **8 turn-by-turn maneuver icons** (depart/straight, left, right, sharp
    left, sharp right, U-turn, arrival flag) — bold arrow style, colour-coded
    (green normal turns, orange sharp, red U-turn, green flag for arrival).
  - **Function icons** — distance (ruler), route (path + dots), navigate
    (compass arrow), waypoint.
  - **Full sidebar set** — 38 duotone pictograms (pin, star, image, bar chart,
    compass, map, book, calendar, wallet, calculator, heart, ticket,
    briefcase, building, houses, warning, bell, people, signpost, clock,
    gear, shield, translate, chat, bookmark, inbox, robot, card, list,
    activity, plus, check-square, close, login, person-plus, chevrons, quote,
    hospital) in one consistent style matching the app palette.
  - Original vector artwork (no third-party license required, no attribution
    burden — unlike Flaticon, whose CDN is also unreachable from this
    sandbox).
- **Sidebar** (`components/layout/Sidebar.jsx`): every link now renders its
  real duotone icon instead of the bare line glyph; new
  **"Distances & Directions"** entry added under Explore. Group chevrons,
  close/login chrome stay line icons (functional controls).
- **Navbar**: keeps its functional line glyphs (menu/bell/search/sun/moon) —
  those are universal chrome and match the admin-dashboard look the sidebar
  theme is built on; every *meaningful* symbol in the app is now a real icon.
- **Registry** `src/utils/uiIcons.jsx`: `UI_ICON()`, `TurnIcon` component,
  `maneuverFromStep()` which normalises any backend step shape
  (`turn` key → `maneuver` key → instruction text) to the right icon.

### 2. Structured turn keys on every route step (backend)
- `navigation/route_engine.py` `build_maneuvers()` now emits a structured
  `turn` per step (`start / left / right / sharp_left / sharp_right / uturn /
  arrive / straight`) computed from the real geometry angles — no invented
  maneuvers.
- `navigation/fallback_providers.py` (offline GraphML corridor routes) maps
  each provider maneuver to the same `turn` keys (`MANEUVER_TO_TURN`), so
  Kathmandu→Dailekh (572 km, 96 steps) now carries
  start → left → straight → … → arrive.
- Frontend turn lists (`Navigation.jsx` HUD + `TurnByTurnNav.jsx`) render the
  real maneuver icons — the arrow now actually shows left/right/straight.

### 3. NEW: `/distances` — Distances & Directions explorer
- **New backend endpoint** `GET /api/v1/destinations/map-points/`:
  all active approved destinations with coordinates as a lightweight feed
  (id, slug, name, district, province, lat, lng, category) — 4,750 points in
  one request, server-cached 5 min (the regular list endpoint caps at 100
  per page and can't serve a whole-country plot).
- **New page** (`src/pages/DistancesExplorer.jsx`, lazy route `/distances`):
  - **Source**: consent-gated "Use my location" (GPS prompt only after the
    explicit click), or any destination via the origin search, or
    Kathmandu (clearly labelled *assumed*).
  - **All destinations, nearest first**: straight-line (haversine) distance +
    16-point bearing + arrow computed client-side for every point; honest
    "straight line" labelling on every row.
  - **Real Nepal map** (Leaflet + OpenStreetMap tiles): all 4,750 points as
    canvas circle markers, origin marker, selected place pin with a distance
    label and a dashed straight line; the road route (when planned) is drawn
    as a solid line and the map auto-fits it.
  - **Search** (Rolpa / Humla / Jumla / Dadeldhura / Rukum Chhota / …)
    filters the ranked list and selecting a place pans + highlights it on the
    map; list rows chunk-render (300 at a time) so thousands of rows stay
    smooth.
  - **Get directions** on any row → existing travel-plan API → road distance,
    travel time, honest grade badge (Road-verified / Corridor estimate /
    Estimate) + full turn-by-turn list with the new real maneuver icons.
- Sidebar entry + migration `0074` register the page in the managed
  navigation (so it survives the admin-managed sidebar filter); i18n keys in
  all 3 languages; shared geo helpers (`haversineKmPrecise`, `compassPoint`,
  `compassArrow`, `formatDistanceKm`) added to `utils/placeUtils.js`.

**Verified:** 515 backend tests pass (navigation + tourist); new endpoint
serves 4,750 points with Rolpa/Humla/Jumla/Dadeldhura/Rukum/Dailekh all
present; corridor route to Dailekh returns 96 structured turn steps;
eslint clean on all touched files; production build green; `/distances` +
icon assets serve 200.
...
GPS position as the start) and get the real road route, distance, travel time,
genuine alternative routes, a per-mode comparison (drive / walk / cycle /
flight), turn-by-turn steps, and one-click hand-off to live navigation.

### Backend (free routing only — public OSRM, no API key)
- `GET/POST /api/v1/navigation/travel-plan/` — resolves both endpoints to
  **real destination coordinates** (slug / id / `lat,lng` / place name),
  delegates to the single central routing engine (OSRM when
  `ROUTING_BASE_URL` is configured, honestly-labelled corridor / straight-line
  fallbacks otherwise). Returns: best route + up to 2 genuine OSRM
  alternatives, per-mode table (measured vs labelled estimates), straight-line
  reference distance, confidence grade (`ROAD-VERIFIED` /
  `CORRIDOR-ESTIMATE` / `ESTIMATE`), and a live-navigation session id.
  Caching (600 s) + rate limiting (429) inherited from the engine.
- `GET /api/v1/navigation/travel-between/?from=<slug>` — "travel from X to
  every other destination": nearest recorded destinations with straight-line
  km (labelled), rough drive-time estimate, and per-province coverage with
  nearest distance. Province spellings normalised (Devanagari / short /
  "X Province" → canonical name). Cached 3600 s.
- 14 new contract tests (resolution, GPS origin, same-place rejection,
  alternatives mapping, 429 rate limit, caching, province normalisation,
  honest fallbacks). **Backend suite: 539/539 OK.**

### Frontend
- New public page **`/travel` — Travel Planner**: destination autocomplete
  (real records only) for both ends, "Use my location" GPS origin, swap,
  mode chips (Drive / Walk / Cycle), best-route hero (distance · time ·
  confidence badge), Leaflet map with the drawn route + destination pin,
  alternative-route cards (click to preview geometry), turn-by-turn list,
  mode comparison table with "Get real route" per mode, save-to-history,
  and "Start live navigation" hand-off to `/navigation`.
  "Travel from here" panel lists the nearest destinations with honest
  straight-line distances and one-click plan.
- **Destination details page**: new "Getting there" card under the map —
  compute the real route from your location (or any other destination) to
  that destination on demand, with confidence badge, straight-line
  comparison, alternative count, and planner / navigation shortcuts.
- Sidebar: "Travel Planner" link (Explore group). Fully i18n'd (English /
  नेपाली / हिन्दी — 38 keys × 3 languages).
- Verification: production build clean, eslint 0 errors on touched files,
  frontend logic tests pass, navigation behaviour tests 130/130.
- E2E pipeline proven against a spec-compliant OSRM server (sandbox has no
  external egress, so a local mock OSRM stood in for
  router.project-osrm.org): Boudhanath Stupa → Dullu Durbar returned
  `source=osrm, grade=real-road, confidence=ROAD-VERIFIED`, 427.9 km / 15 h,
  8 turn-by-turn steps, 2 alternatives, and a live-navigation session id.
