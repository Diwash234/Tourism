...
## 📧 Real Email, SMS & OAuth — "Does it really send the code?"

All verification flows exist end-to-end (register ➔ verification link ➔ verify;
phone OTP ➔ verify; password reset; OAuth callbacks). Whether a message is
**physically delivered** depends on `Tourism/.env`:

| Channel | Default (no setup) | To deliver for real |
|---|---|---|
| **Email** (verification links, resets, notifications) | `EMAIL_BACKEND=...console.EmailBackend` — the email is generated and printed to the **server log** (read it there during development) | Set SMTP in `.env` — e.g. Gmail: enable 2-Step Verification, create an **App Password**, then `EMAIL_BACKEND=django.core.mail.backends.smtp.EmailBackend`, `EMAIL_HOST=smtp.gmail.com`, `EMAIL_PORT=587`, `EMAIL_USE_TLS=True`, `EMAIL_HOST_USER=you@gmail.com`, `EMAIL_HOST_PASSWORD=<16-char app password>`, `DEFAULT_FROM_EMAIL=you@gmail.com` and restart. Any SMTP provider (Mailgun/SES) works the same way. |
