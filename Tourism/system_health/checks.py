...
def _backup_check(max_age_hours: float = 24.0) -> dict:
    """Backup freshness: 🔴 when no archive exists or the newest is stale."""
    import os
    from datetime import datetime, timezone as _tz
    from pathlib import Path
    backups_dir = Path(str(settings.BASE_DIR)) / "backups"
    archives = sorted(backups_dir.glob("*.gz")) if backups_dir.exists() else []
    if not archives:
        return {"ok": False, "warning": "BACKUP WARNING", "last_backup_at": None,
                "last_backup_age_hours": None, "archives": 0,
                "note": "No backup archives found. Run: manage.py backup_database"}
    newest = max(archives, key=lambda p: p.stat().st_mtime)
    age_hours = (datetime.now(_tz.utc).timestamp() - newest.stat().st_mtime) / 3600.0
    if age_hours > max_age_hours:
        return {"ok": False, "warning": "BACKUP WARNING",
                "last_backup_at": datetime.fromtimestamp(newest.stat().st_mtime, _tz.utc).isoformat(),
                "last_backup_age_hours": round(age_hours, 2), "archives": len(archives),
                "note": f"Newest backup is {age_hours:.1f} h old (limit {max_age_hours:.0f} h)."}
...
def run_all_checks() -> dict[str, Any]:
    ...
    dhm = _http_check(settings.DHM_FEED_URL, timeout=3.0) if settings.DHM_FEED_URL else {"ok": False, "configured": False, "note": "DHM feed not configured"}
    bipad = _http_check(settings.BIPAD_FEED_URL, timeout=3.0) if settings.BIPAD_FEED_URL else {"ok": False, "configured": False, "note": "BIPAD feed not configured"}
    routing_url = (getattr(settings, "ROUTING_BASE_URL", "") or getattr(settings, "ROUTING_API_URL", "")).strip()
    routing = _http_check(routing_url, timeout=3.0) if routing_url else {"ok": False, "configured": False, "note": "Road routing not configured"}
    backup = _backup_check()

    overall_ok = db["ok"] and disk["ok"] and media["ok"] and backup["ok"] and dhm["ok"] and bipad["ok"] and routing["ok"]
