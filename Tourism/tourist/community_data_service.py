...
def export_verified_ml_feedback():
    ...
    # Automatically trigger asynchronous/subprocess retraining for ML models when feedback is exported
    try:
        ml_dir = ROOT / "ml_service"
        subprocess.Popen([sys.executable, "training/train_budget_model.py"], cwd=ml_dir)
        subprocess.Popen([sys.executable, "training/train_risk_model.py"], cwd=ml_dir)
        subprocess.Popen([sys.executable, "training/train_recommendation_model.py"], cwd=ml_dir)
    except Exception:
        pass
