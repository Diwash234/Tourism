...
def upsert_destination_location(destination):
    """Patch the legacy overlay only when explicitly enabled.

    The overlay is not a release source.  Normal admin edits must not create a
    second mutable JSON projection; the versioned catalog is regenerated by
    ``build_verified_snapshot`` instead.
    """
    if os.environ.get("NEPAL_YATRA_ENABLE_LEGACY_JSON", "") != "1":
        return None
...
def sync_admin_destination_json(destination):
    """Do not create a legacy JSON mirror during normal admin edits.

    ``verified_tourism_data.json`` is the explicit, versioned release
    artifact.  A mutable per-edit mirror previously left E2E ghosts and
    stale IDs behind, so legacy projection writes are opt-in for migrations.
    """
    if os.environ.get("NEPAL_YATRA_ENABLE_LEGACY_JSON", "") != "1":
        return None
