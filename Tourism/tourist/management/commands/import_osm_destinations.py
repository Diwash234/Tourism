    ...
    def handle(self, *args, **options):
        ...
        for i, row in enumerate(rows, 1):
            try:
                ...
                # Enrich missing fields; NEVER overwrite admin-controlled
                # values — differing incoming values become ImportConflict
                # rows for the admin queue instead (admin = source of truth).
                from django.utils import timezone as _tz
                from tourist.models import ImportConflict

                update = []
                snapshot = dict(dest.imported_data or {})
                incoming = {}
                if cat:
                    incoming["category"] = cat.name
                if city:
                    incoming["city"] = city
                if district:
                    incoming["district"] = district
                if province:
                    incoming["province"] = province
                if lat:
                    incoming["latitude"] = f"{float(lat):.6f}"
                if lon:
                    incoming["longitude"] = f"{float(lon):.6f}"

                def _current(field):
                    if field == "category":
                        return dest.category.name if dest.category_id else ""
                    val = getattr(dest, field, None)
                ...
                for field, new_val in incoming.items():
                    ...
                    if not cur:
                        if field == "category":
                            ...
                            setattr(dest, field, lat if field == "latitude" else (
                                lon if field == "longitude" else new_val))
                        snapshot[field] = new_val
                        update.append(field)
                    elif cur == new_val:
                        snapshot[field] = new_val  # same value re-imported
                    else:
                        # Non-empty, differing value -> never touch the field;
                        # record an ImportConflict for the admin queue unless
                        # this exact incoming value was already ruled on.
                        exists = ImportConflict.objects.filter(
                            destination=dest, field=field,
                            incoming_value=new_val).exists()
                        if not exists:
                            ImportConflict.objects.create(
                                destination=dest, field=field,
                                current_value=cur, incoming_value=new_val,
                                source="osm-csv")
                            conflicts_created[0] += 1
                ...
                # NOTE: status/is_active are admin decisions — a re-import must
                # never re-publish an archived/rejected record.

                if snapshot != (dest.imported_data or {}):
                    dest.imported_data = snapshot
                    dest.imported_at = _tz.now()
                    update.extend(["imported_data", "imported_at"])
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Done. created={created} enriched={enriched} unchanged={unchanged} "
            f"import_conflicts={conflicts_created[0]} total_destinations={Destination.objects.count()}"
        ))
