...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # staff-ops runs the live E2E staff workspace: grant exactly the
        # modules the staff-ops endpoints require (approve stays excluded so
        # the 403 guards remain meaningful).
        from tourist.models import StaffCapabilityProfile
        ops = User.objects.filter(email="staff-ops@nepaltourism.com").first()
        if ops:
            StaffCapabilityProfile.objects.update_or_create(user=ops, defaults={
                "capabilities": {
                    "dashboard": ["view", "change"], "hotels": ["view", "change"],
                    "support": ["view", "change"], "feedback": ["view", "change"],
                    "tasks": ["view", "change"], "safety": ["view", "change"],
                    "destinations": ["view", "add", "change"],
                    "images": ["view", "add"],
                }})
