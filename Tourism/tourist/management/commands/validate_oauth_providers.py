"""OAuth provider configuration smoke test (§16).

Validates that the SUPPLIED credentials are actually accepted by each
provider — without fabricating anything and without printing secrets:
  ...
  * a provider that recognises the client answers invalid_grant (Google)
    or a bad-verification-code error (GitHub) => CONFIG OK;
  * unauthorized_client / unknown app => CONFIG INVALID (exit 1);
  * unconfigured providers are reported as SKIPPED (buttons stay honestly
    disabled until credentials are supplied).

Usage:
    python manage.py validate_oauth_providers
"""
import json
        ...
        if settings.GITHUB_CLIENT_ID and settings.GITHUB_CLIENT_SECRET:
            try:
                resp = requests.post(
                    ...
                    data={"code": "deliberately-invalid-code",
                ...
                # GitHub: known app + bad code -> "bad_verification_code";
                # unknown app -> 404 page / different error.
