...
class UserRouteCalculateView(APIView):
    ...
    def post(self, request):
        ...
        # Destination is resolved BEFORE origin validation: an unresolvable
        # destination is an honest 404 (DESTINATION_UNRESOLVED) regardless of
        # the origin, so the client learns the place does not exist instead
        # of a generic origin error.
        dest_city = ""
        if destination is not None:
            ...
            if not resolved_destination:
                return Response(
                    {"detail": f"Could not resolve the destination '{dest_name or dest_slug}' to a real location.",
            ...
            return Response(
                {"detail": "A destination is required: send destination_id, destination_slug, or destination_name."},
        ...
        route_source = {
            "ROUTED": "osrm",
            "GRAPH_APPROXIMATION": "graphml_fallback",
            "STRAIGHT_LINE": "straight_line_fallback",
        }.get(confidence, "straight_line_fallback")
        for step in steps:
            if step.get("maneuver_grade") in (None, ""):
                step["maneuver_grade"] = {"depart": "A", "arrive": "A"}.get(step.get("maneuver", ""), "B")
        ...
        if geometry_kind == "routed" and not steps and len(geometry_coordinates) >= 2:
            steps = []
            for i, (plat, plng) in enumerate(geometry_coordinates):
                if i == 0:
                    instr, maneuver = "Head out along the mapped corridor", "depart"
                elif i == len(geometry_coordinates) - 1:
                    instr, maneuver = "Arrive at your destination", "arrive"
                else:
                    instr, maneuver = "Continue along the mapped corridor", "continue"
                steps.append({
                    "instruction": instr, "maneuver": maneuver,
                    "maneuver_grade": {"depart": "A", "arrive": "A"}.get(maneuver, "B"),
                    "point": [plat, plng],
                })
...
# (OSRM when configured, labelled fallbacks otherwise).
# ============================================================
...
_MODE_ESTIMATES = {
    ...
    "walking":    {"road_factor": 1.30, "avg_kmh": 4.2,  "label": "walk"},
    "cycling":    {"road_factor": 1.25, "avg_kmh": 13.0, "label": "cycle"},
    "hiking":     {"road_factor": 1.45, "avg_kmh": 3.2,  "label": "hike"},
    "motorcycle": {"road_factor": 1.22, "avg_kmh": 38.0, "label": "ride"},
    "flight":     {"road_factor": 1.02, "avg_kmh": 560.0, "label": "fly"},
}

# source (as emitted by the route providers) -> honest grade label
_SOURCE_GRADE = {
    "osrm": "real-road",
    "graphml_fallback": "corridor-estimate",
    "bundled_graph": "corridor-estimate",
    "straight_line_fallback": "straight-line-estimate",
    "straight_line": "straight-line-estimate",
}


def _parse_point_param(value):
    """`lat,lng` string -> (lat, lng) or None."""
    if value in (None, ""):
        return None
    try:
...
def _resolve_destination_param(value):
    ...
    target = value
    if target.lower().startswith("dest:"):
    ...
    if target.isdigit():
        dest = Destination.publicly_visible().filter(pk=int(target)).first()
    else:
        dest = Destination.publicly_visible().filter(slug=target).first()
    ...
    if dest is not None:
        ...
        return {
            "name": dest.name, "kind": "destination", "id": dest.id, "slug": dest.slug,
            "latitude": float(dest.latitude), "longitude": float(dest.longitude),
            "province": dest.province or "", "district": dest.district or "",
            "coordinate_status": dest.coordinate_status or "",
        }, None

    point = _parse_point_param(value)
    if point:
        return {"name": f"Point ({point[0]:.5f}, {point[1]:.5f})", "kind": "point",
                "id": None, "slug": None, "latitude": point[0], "longitude": point[1],
                "province": "", "district": "", "coordinate_status": "user-provided"}, None

    resolved = LocationSearchService.resolve_single_place(value)
    if resolved and resolved.get("latitude") is not None:
        return {
            "name": resolved.get("name") or value, "kind": "place",
            "id": resolved.get("id"), "slug": resolved.get("slug"),
            "latitude": float(resolved["latitude"]), "longitude": float(resolved["longitude"]),
            "province": resolved.get("province") or resolved.get("city") or "",
            "district": resolved.get("district") or "",
            "coordinate_status": resolved.get("coordinate_status") or "geocoded",
        }, None

    return None, (Response({
        "detail": f"'{value}' could not be resolved to a real place with coordinates.",
        "error": "endpoint_unresolved",
    }, status=status.HTTP_404_NOT_FOUND))


class TravelPlannerView(APIView):
    """GET/POST /api/v1/navigation/travel-plan/

    The tourist's route planner between ANY two recorded destinations
    (or a place name, or the user's live GPS fix as origin):

        /navigation/travel-plan/?destination=pokhara-lakeside
            &origin=kathmandu-durbar-square&mode=driving

    Returns the best real route for the requested mode, genuine
    alternatives when the road router can compute them, a per-mode
    ...
    def post(self, request):
        qp = {**request.query_params, **request.data}
        return self._plan(request, qp)

    def get(self, request):
        return self._plan(request, request.query_params)

    def _plan(self, request, qp):
        from django.core.cache import cache
        import hashlib

        mode = (qp.get("mode") or "driving").strip().lower()
        if mode not in _PLANNER_MODES:
            return Response(
                {"detail": f"Unsupported mode '{mode}'.",
                 "supported": list(_PLANNER_MODES)},
                status=status.HTTP_400_BAD_REQUEST)
        want_alts = str(qp.get("alternatives", "1")).lower() not in ("0", "false", "no")

        # ---- origin: explicit coords win, then endpoint spec ------------
        origin_lat = qp.get("origin_lat") or qp.get("start_latitude")
        origin_lng = qp.get("origin_lng") or qp.get("start_longitude")
        origin_err = None
        origin = None
        if origin_lat not in (None, "") and origin_lng not in (None, ""):
            try:
                origin = {"name": qp.get("origin_name") or "Current Location",
                          "kind": "point", "id": None, "slug": None,
                          "latitude": float(origin_lat), "longitude": float(origin_lng),
                          "province": "", "district": "",
                          "coordinate_status": "gps" if qp.get("origin_name") else "user-provided"}
            except (TypeError, ValueError):
                return Response({"detail": "origin_lat/origin_lng must be numeric."},
                                status=status.HTTP_400_BAD_REQUEST)
        else:
            origin, origin_err = _resolve_destination_param(qp.get("origin"))
            if origin_err:
                return origin_err
            if origin is None:
                return Response({
        ...
        destination, dest_err = _resolve_destination_param(qp.get("destination"))
        if dest_err:
            return dest_err
        if destination is None:
            return Response(
                {"detail": "destination is required (slug, id, or place name).",
                 "error": "destination_required"},
                status=status.HTTP_400_BAD_REQUEST)

        start = (origin["latitude"], origin["longitude"])
        dest_pt = (destination["latitude"], destination["longitude"])
        if haversine_distance_km(*start, *dest_pt) is not None and \
                haversine_distance_km(*start, *dest_pt) < 0.015:
            return Response(
                {"detail": "Origin and destination are the same place.",
                 "error": "same_place"},
                status=status.HTTP_400_BAD_REQUEST)

        quant = lambda v: f"{float(v):.3f}"
        cache_key = "travel-plan:" + hashlib.sha256(":".join([
            quant(origin["latitude"]), quant(origin["longitude"]),
            quant(destination["latitude"]), quant(destination["longitude"]), mode,
        ]).encode()).hexdigest()
        cached = cache.get(cache_key)
        if cached is not None:
            cached["cached"] = True
        ...
        from navigation import navigation_service, route_engine
        try:
            result, route_cached = route_engine.cached_route(
        ...
        route = result["route"]
        straight_km = haversine_distance_km(*start, *dest_pt) or 0.0
        source = route.get("source", "unknown")

        # live-navigation session for hand-off to /navigation (best-effort)
        route_id = None
        try:
        ...
        modes_table = []
        for m in ("driving", "walking", "cycling", "flight"):
            if m == mode:
                modes_table.append({
                    "mode": m, "label": _MODE_ESTIMATES.get(m, {}).get("label", m),
                    "distance_m": round(float(route["distance_m"]), 1),
                    "duration_s": round(float(route["duration_s"]), 1),
                    "source": source,
                    "grade": _SOURCE_GRADE.get(source, "estimate"),
                    "real_available": True,
                })
            else:
                est = _MODE_ESTIMATES[m]
                est_m = straight_km * 1000.0 * est["road_factor"]
                est_s = est_m / max(1.0, est["avg_kmh"]) * 3.6
                modes_table.append({
                    "mode": m, "label": est["label"],
                    "distance_m": round(est_m, 1),
        ...
        real_rows = [r for r in modes_table if r["grade"] == "real-road"]
        best_row = (real_rows or modes_table)[0]
        best = {
            "mode": best_row["mode"],
            "distance_m": best_row["distance_m"],
            "duration_s": best_row["duration_s"],
        ...
        payload = {
            ...
            "primary": {
                ...
                "duration_min": max(1, int(round(float(route["duration_s"]) / 60.0))),
                "source": source,
                "grade": _SOURCE_GRADE.get(source, "estimate"),
            ...
            "alternatives": [
                {
                    "distance_m": round(float(a.get("distance_m", 0)), 1),
                    "distance_km": round(float(a.get("distance_m", 0)) / 1000.0, 1),
                    "duration_min": max(1, int(round(float(a.get("duration_s", 0)) / 60.0))),
                    "source": a.get("source", ""),
                    "note": a.get("note") or "",
            ...
            "modes": modes_table,
            "best": best,
            "confidence": {"osrm": "ROAD-VERIFIED",
                           "graphml_fallback": "CORRIDOR-ESTIMATE",
                           "bundled_graph": "CORRIDOR-ESTIMATE"}.get(source, "ESTIMATE"),
...
class TravelBetweenDestinationsView(APIView):
    """GET /api/v1/navigation/travel-between/?from=<slug|id>&limit=24
    ...
    def get(self, request):
        ...
        origin = (request.query_params.get("from")
                  or request.query_params.get("destination") or "").strip()
        if not origin:
            return Response({"detail": "from=<destination slug or id> is required."},
                            status=status.HTTP_400_BAD_REQUEST)
        try:
            limit = max(1, min(int(request.query_params.get("limit", 24)), self.MAX_LIMIT))
        except ValueError:
            limit = 24
        ...
        origin_target = origin[5:].strip() if origin.lower().startswith("dest:") else origin
        if origin_target.isdigit():
            from_dest = Destination.publicly_visible().filter(pk=int(origin_target)).first()
        else:
            from_dest = Destination.publicly_visible().filter(slug=origin_target).first()
        if from_dest is None or from_dest.latitude is None or from_dest.longitude is None:
            return Response(
                {"detail": f"'{origin}' is not a public destination with coordinates."},
                status=status.HTTP_404_NOT_FOUND)

        # Normalize the mixed province spellings in the data (English short,
        # "X Province", and Devanagari) to the one canonical display name.
        alias_to_display = {}
        for display, aliases in ProvinceNavigationView.PROVINCES:
            for a in aliases:
                alias_to_display[a] = display

        def canon_province(value):
            v = (value or "").strip()
            if not v:
                return ""
            if v in alias_to_display:
        ...
        o_lat, o_lng = float(from_dest.latitude), float(from_dest.longitude)
        rows = list(
            Destination.publicly_visible()
            .exclude(pk=from_dest.pk)
            .exclude(latitude=None).exclude(longitude=None)
            .values("id", "name", "slug", "latitude", "longitude",
                    "province", "district", "category__name", "views_count")
        )

        province_stats = {}
        total = 0
        for r in rows:
            km = haversine_distance_km(o_lat, o_lng, float(r["latitude"]), float(r["longitude"]))
            if km is None:
                continue
            total += 1
            r["_km"] = round(km, 1)
            r["_drive_min"] = max(1, round(km * 1.22 / 32.0 * 60.0))
            prov = canon_province(r["province"]) or "Unknown"
            st = province_stats.setdefault(prov, {"count": 0, "nearest_km": km})
            st["count"] += 1
            st["nearest_km"] = min(st["nearest_km"], km)

        rows.sort(key=lambda r: r["_km"])
        nearest = []
        for r in rows[:limit]:
            nearest.append({
                "id": r["id"], "name": r["name"], "slug": r["slug"],
                "province": canon_province(r["province"]), "district": r["district"] or "",
                "category": r["category__name"] or "",
                "straight_line_km": r["_km"],
                "drive_estimate_min": r["_drive_min"],
                "distance_note": "Straight-line; open Travel Planner for the real road route.",
            })

        payload = {
            "status": "success",
            "origin": {
                "id": from_dest.id, "name": from_dest.name, "slug": from_dest.slug,
                "latitude": o_lat, "longitude": o_lng,
                "province": canon_province(from_dest.province),
                "district": from_dest.district or "",
            },
            "count_with_coordinates": total,
            "provinces": {
                p: {"count": v["count"], "nearest_km": round(v["nearest_km"], 1)}
                for p, v in sorted(province_stats.items())
            },
            "nearest": nearest,
            "note": ("Distances are straight-line (haversine). Road distance and "
                     "turn-by-turn come from /api/v1/navigation/travel-plan/."),
        }
...
# ---------------------------------------------------------------------------
# Travel options (merged from devin dark-mode-compat layer):
# per-mode comparison, honest fallbacks, provider turn steps.
# ---------------------------------------------------------------------------
