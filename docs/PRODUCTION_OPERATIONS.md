...
## CMS operations (content lifecycle, import protection, duplicates)

Admin content management runs entirely through the Admin Dashboard
(Content Lifecycle CMS tab) on top of these backend endpoints — no direct
database access is ever required:

| Area | Endpoint | Notes |
|---|---|---|
| All content (search/filter/sort/paginate) | `GET /api/v1/admin/destinations` | q, status, district, category, provenance, missing, ordering, page |
| Edit (approval workflow) | `PUT /api/v1/admin/destinations/<id>` | editors without approve capability create a proposal instead of mutating; `reason` recorded; provenance flips to `manual` |
| Approval Center | `GET/POST /api/v1/admin/approvals/` | field diffs; approve/reject/send_back; approve applies + versions + audits |
| Import conflicts | `GET/POST /api/v1/admin/import-conflicts/` | keep/accept/edit with mandatory reason; audited |
| Bulk lifecycle | `POST /api/v1/admin/destinations/bulk/` | publish/unpublish/archive/restore/verify/unverify/delete/assign_category; reason required; delete needs confirm; ≤500 ids; per-record audit |
| Duplicates | `GET /api/v1/admin/duplicates/`, `…/compare/`, `POST …/decision/` | tiers; merge picks survivor, moves images, snapshots absorbed record; not_duplicate dismissals persist |
| Revisions | `GET/POST /api/v1/admin/destinations/revisions/` | list/diff/snapshot/restore; restore is itself versioned + audited |
| Data quality | `GET /api/v1/admin/data-integrity/` | every count links to a filtered admin view |

Import rule (enforced by `import_osm_destinations`): external data only
fills blank fields; any differing value becomes an `ImportConflict` for the
admin queue; archived/rejected records are never re-published by imports.

### Validation & backup commands

```bash
python manage.py validate_production_config      # exits 1 on any critical; never prints secrets
python manage.py validate_oauth_providers        # probes real Google/GitHub endpoints with supplied credentials
python manage.py backup_database --keep 7        # gzip + SHA-256 sidecar + verify + rotation
python manage.py restore_database --file backups/backup-*.sqlite.gz --target /tmp/drill.sqlite3   # rehearsal
python manage.py restore_database --file backups/backup-*.sqlite.gz --confirm                     # live (auto pre-restore backup)
```

Additional cron entries:

```cron
30 3 * * 0 cd /app/Tourism && /app/.venv/bin/python manage.py backup_database --keep 7
0 4 1 * * cd /app/Tourism && /app/.venv/bin/python manage.py restore_database --file "$(ls -t /app/Tourism/backups/*.gz | head -1)" --target /tmp/monthly-drill.sqlite3
```
