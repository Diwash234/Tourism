...
|---|---|---|---|---|---|
| Real OSRM road routing in production | PARTIAL | Full provider + 7/7 validation via local mock; env-configured. Offline fallback upgraded 2026-09-20: bundled-corridor graph routing fixed (dict-vs-list bug) — `graphml_fallback` now yields real multi-step turn-by-turn (left/right/slight maneuvers + landmark names, e.g. Pokhara→Bandipur 18 waypoints/19 steps), still honestly labelled node-level | Network access to an OSRM server | Labelled `graphml_fallback` (waypoint turn-by-turn) / `straight_line_fallback` (last resort) | `ROUTING_BASE_URL` on host → `validate_navigation_routes` → `--write-baseline` |
