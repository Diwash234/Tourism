...
| CMS full-page coverage | Migration `0071` registers the last public pages (districts list/detail, guides, tourism jobs, 4 auth screens, 404) — **58 ManagedPages, every public route CMS-controlled**; `CMSPageIntro` now mounted on districts, district detail, guides, jobs, auth, 404 and staff desk. CMSPanel gained a **media-library picker** (Browse button on section image, media URL and page social-image fields) served by the moderated `admin/media-library/` endpoint. |
