...
| Offline route packs | **Phase 6**: every successful calculation is cached as an offline pack (localStorage, newest-first, same-destination replaced, capped at 5) — route geometry, steps, totals and provenance included. The Navigation HUD lists packs with Load/Delete; loading one restores map/steps/summary under an explicit amber **"Offline copy — calculated {timestamp}. Not a live route."** badge, and a failed calculation while `navigator.onLine === false` points to a matching pack. Storage layer is corruption-safe (bad JSON → `[]`, routeless packs filtered, full/blocked storage degrades silently). Map tiles are deliberately NOT cached — not bundleable honestly. 7 new jsdom checks on the real module (162/162). |
| Waypoint markers + travel-options parity | **Phase 6+**: multi-stop routes now draw **numbered stop markers** on the map (MapView `waypoints` prop, teal numbered icons, popups naming each stop; coordinates come from the API's resolved `waypoints`, restored from offline packs too), and `/navigation/travel-options/` accepts the **same waypoints contract** — distances/durations become per-leg route-metric sums (`multi_stop` + `waypoints` echoed in the response, via-corridor figures never silently reuse the direct distance — regression-checked against it). TravelOptionsPanel passes the page's stops through. 3 new backend tests (224 OK) + 1 new e2e check (66/66). |
...
- Backend: `tourist.tests_regression` — **224 tests OK** (district architecture ×7,
...
- Live e2e: **66/66 passed**, including district API (curated + auto-composed
...
- Nav behaviour harness (`npm run test:nav`, jsdom, real components):
  **162/162 passed**, including 24 live-navigation checks and 7 offline-pack checks (following/
