...
test.describe("Himal AI published packages", () => {
  test("5-day under $500 returns the live published circuit, not invented or over-budget offers", async ({ page }) => {
    ...
    await expect(page.getByText("Himalayan Luxury Panorama Week")).toHaveCount(0)
    await expect(page.getByText("Pokhara Eco-Lodge Pending Stay")).toHaveCount(0)
    await expect(page.getByText("Kathmandu & Pokhara 6-Day Cultural Circuit").first()).toBeVisible()
