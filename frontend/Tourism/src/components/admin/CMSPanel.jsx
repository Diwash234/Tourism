  ...
  return <div className="space-y-4 text-slate-100">
    ...
    <div className="grid xl:grid-cols-[220px_300px_1fr] gap-4">
      ...
      <section className="bg-slate-950 border border-slate-700 rounded-2xl overflow-hidden h-fit max-h-[72vh]">
        <div className="p-3 border-b border-slate-800 flex justify-between"><b className="capitalize">{resource} <span className="ml-1 text-xs font-normal text-emerald-600">({rows.length})</span></b><button onClick={() => load()} title="Refresh"><FiRefreshCw/></button></div>
      ...
      <section className="bg-slate-950 border border-slate-700 rounded-2xl p-4 min-w-0">
        {!selected ? <p className="text-slate-500 py-20 text-center">Select a record or create a new draft.</p> : <div className="space-y-3">
          ...
          <details className="rounded-xl border border-emerald-200 bg-white p-3"><summary className="cursor-pointer text-xs font-black text-emerald-800">Advanced structured JSON</summary><textarea rows="12" spellCheck="false" value={json} onChange={event => setJson(event.target.value)} className="mt-3 w-full rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-900 font-mono text-xs p-3"/></details>
          <p className="text-[11px] text-slate-500">Use the fields above for normal editing. Only allowlisted structured fields are saved; executable HTML, CSS and scripts are rejected.</p>
