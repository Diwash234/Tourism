...
export default function DataExplorerPanel() {
  ...
  const [genericEdit,setGenericEdit]=useState(null),
  [resource,setResource]=useState("destinations"), [query,setQuery]=useState(""), [page,setPage]=useState(1), [data,setData]=useState({results:[],count:0,total_pages:1}), [loading,setLoading]=useState(false)
  ...
  const openDestination=async(id)=>{if(resource!=="destinations")return;try{const{data}=await adminApi.getAdminDestination(id);setDetail(data);setEdit({name:data.name||"",short_description:data.short_description||"",description:data.description||"",city:data.city||"",district:data.district||"",province:data.province||"",municipality:data.municipality||"",ward_number:data.ward_number??"",latitude:data.latitude??"",longitude:data.longitude??"",altitude:data.altitude||"",best_time_to_visit:data.best_time_to_visit||"",opening_hours:data.opening_hours||"",entry_fee:data.entry_fee??"",history:data.history||"",cultural_significance:data.cultural_significance||"",food_cuisine_info:data.food_cuisine_info||"",cover_image_url:(typeof data.cover_image==="string"?data.cover_image:data.cover_image?.url)||""})}catch{showToast("Could not load destination details","error")}}
  const openRow=async(id)=>{if(!id)return;if(resource==="destinations"){openDestination(id);return}try{const[sch,row]=await Promise.all([adminApi.exploreSchema(resource),adminApi.exploreRow(resource,id)]);setGenericEdit({id,fields:sch.data.editable||[],values:row.data.values||{}})}catch{showToast("Could not load record for editing","error")}}
  const saveGenericEdit=async()=>{try{const{data}=genericEdit.id?await adminApi.updateExploreRow({resource,id:genericEdit.id,fields:genericEdit.values}):await adminApi.createExploreRow({resource,fields:genericEdit.values});showToast(genericEdit.id?(data.changed?.length?`Updated: ${data.changed.join(", ")}`:"No changes to save"):"Record created","success");setGenericEdit(null);load()}catch(e){showToast(e.response?.data?.detail||(genericEdit.id?"Update failed":"Create failed"),"error")}}
  const deleteGenericEdit=async()=>{if(!genericEdit?.id)return;if(!window.confirm(`Delete this ${resource.replace(/_/g," ")} record permanently? Records referenced by other data are refused.`))return;try{const{data}=await adminApi.deleteExploreRow(resource,genericEdit.id);showToast(data.message||"Record deleted","success");setGenericEdit(null);load()}catch(e){showToast(e.response?.data?.detail||"Delete failed","error")}}
  const startGenericCreate=async()=>{try{const sch=await adminApi.exploreSchema(resource);setGenericEdit({id:null,fields:sch.data.editable||[],values:{}})}catch{showToast("Could not load form fields","error")}}
  ...
  const startCreate=()=>{setDetail({id:null,name:""});setEdit({name:"",short_description:"",description:"",city:"",district:"",province:"",municipality:"",ward_number:"",latitude:"",longitude:"",altitude:"",best_time_to_visit:"",opening_hours:"",entry_fee:"",history:"",cultural_significance:"",food_cuisine_info:"",cover_image_url:""})}
  ...
  return <div className="grid lg:grid-cols-[270px_1fr] gap-5">
    ...
    <section className="rounded-2xl bg-slate-950 border border-slate-700 p-5 overflow-hidden"><div className="flex flex-wrap gap-2 justify-between mb-4"><div><h2 className="text-xl text-white font-black flex items-center gap-2"><FiDatabase/>Database Explorer</h2><p className="text-xs text-slate-400">{data.count} records · click destinations for details</p></div><div className="flex"><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&load()} className="rounded-l-xl bg-slate-800 border border-slate-600 px-3 text-sm text-white" placeholder="Search records…"/><button onClick={load} className="rounded-r-xl bg-amber-400 px-4"><FiSearch/></button></div></div>
      ...
      {resource!=="destinations"&&<button onClick={startGenericCreate} className="text-xs bg-emerald-600 text-white font-black rounded-xl px-4 py-2 mb-3">+ Add {resource.replace(/_/g," ")}</button>}
      <div className="overflow-auto max-h-[45vh]"><table className="min-w-full text-xs"><thead className="sticky top-0 bg-slate-900"><tr>{data.columns?.map(c=><th key={c} className="text-left text-slate-400 px-3 py-2 whitespace-nowrap">{c}</th>)}</tr></thead><tbody>{loading?<tr><td className="text-slate-400 p-5">Loading…</td></tr>:data.results.map((row,i)=><tr onClick={()=>openRow(row.id)} key={row.id||i} className="border-t border-slate-800 cursor-pointer hover:bg-slate-800">{data.columns?.map(c=><td key={c} className="text-slate-300 px-3 py-2 max-w-72 truncate">{row[c]===null?'—':String(row[c]??'')}</td>)}</tr>)}</tbody></table></div><div className="flex items-center justify-between mt-3 text-xs text-slate-400"><button disabled={page<=1} onClick={()=>load(page-1)} className="px-3 py-2 bg-slate-800 rounded disabled:opacity-30">Previous</button><span>Page {page} of {data.total_pages||1}</span><button disabled={page>=(data.total_pages||1)} onClick={()=>load(page+1)} className="px-3 py-2 bg-slate-800 rounded disabled:opacity-30">Next</button></div>
      {genericEdit&&<div className="mt-5 rounded-2xl bg-slate-900 border border-sky-500/40 p-5"><div className="flex justify-between"><h3 className="text-white text-lg font-black">{genericEdit.id?`Edit ${resource.replace(/_/g," ")} #${genericEdit.id}`:`Add ${resource.replace(/_/g," ")}`}</h3><button onClick={()=>setGenericEdit(null)} className="text-slate-400"><FiX/></button></div><div className="grid md:grid-cols-2 gap-3 mt-4 max-h-[50vh] overflow-y-auto">{genericEdit.fields.map(f=>{
        const labelCls="text-[10px] uppercase text-slate-400 font-bold"
        const val=genericEdit.values[f.name]
        const setV=(v)=>setGenericEdit({...genericEdit,values:{...genericEdit.values,[f.name]:v}})
        if(f.type==="boolean")return <label key={f.name} className={`${labelCls} flex items-center gap-2`}><input type="checkbox" checked={!!val} onChange={e=>setV(e.target.checked)}/>{f.name.replace(/_/g," ")}</label>
