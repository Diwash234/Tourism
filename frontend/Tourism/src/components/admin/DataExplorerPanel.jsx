...
export default function DataExplorerPanel() {
  ...
  const saveGenericEdit=async()=>{try{const{data}=await adminApi.updateExploreRow({resource,id:genericEdit.id,fields:genericEdit.values});showToast(data.changed?.length?`Updated: ${data.changed.join(", ")}`:"No changes to save","success");setGenericEdit(null);load()}catch(e){showToast(e.response?.data?.detail||"Update failed","error")}}
  ...
  return <div className="grid lg:grid-cols-[270px_1fr] gap-5">
    ...
    <section className="rounded-2xl bg-slate-950 border border-slate-700 p-5 overflow-hidden"><div className="flex flex-wrap gap-2 justify-between mb-4"><div><h2 className="text-xl text-white font-black flex items-center gap-2"><FiDatabase/>Database Explorer</h2><p className="text-xs text-slate-400">{data.count} records · click destinations for details</p></div><div className="flex"><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&load()} className="rounded-l-xl bg-slate-800 border border-slate-600 px-3 text-sm text-white" placeholder="Search records…"/><button onClick={load} className="rounded-r-xl bg-amber-400 px-4"><FiSearch/></button></div></div>
      ...
      {genericEdit&&<div className="mt-5 rounded-2xl bg-slate-900 border border-sky-500/40 p-5"><div className="flex justify-between"><h3 className="text-white text-lg font-black">Edit {resource.replace(/_/g," ")} #{genericEdit.id}</h3><button onClick={()=>setGenericEdit(null)} className="text-slate-400"><FiX/></button></div><div className="grid md:grid-cols-2 gap-3 mt-4 max-h-[50vh] overflow-y-auto">{genericEdit.fields.map(f=>{
      ...
      })}</div><div className="flex gap-2 mt-4"><button onClick={saveGenericEdit} className="px-5 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-sm font-black">Save changes</button><button onClick={()=>setGenericEdit(null)} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 text-sm font-bold">Cancel</button></div></div>}
