...
export default function MarketplacePanel() {
  ...
  const saveListing = async (event) => {
    event.preventDefault()
    try {
      const partnerId = listingForm.partner_id || approvedPartners[0]?.id
      ...
      showToast("Offer published to packages page!", "success")
    ...
    <div className="space-y-6" data-testid="marketplace-panel">
      ...
      {tab === "orders" && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="text-sm text-slate-300 mb-4">These are booking requests or hand-offs to partner sites. Confirm after the operator accepts payment offline.</p>
