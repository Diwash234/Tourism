...
export default function MediaLibraryPanel() {
  ...
  const handleOpenEditMedia = (image) => {
    ...
    setEditForm({
      ...
      external_url: image.url || image.external_url || "",
  ...
  const handleSaveEditMedia = async (e) => {
    ...
    try {
      if (replacementFile) {
        ...
        body.append("id", editingMedia.id)
        body.append("file", replacementFile)
        if (editForm.caption) body.append("caption", editForm.caption)
        if (editForm.alt_text) body.append("alt_text", editForm.alt_text)
        if (editForm.photographer) body.append("photographer", editForm.photographer)
        if (editForm.license_type) body.append("license_type", editForm.license_type)
        await adminApi.updateMediaLibrary(body)
      } else {
        await adminApi.updateMediaLibrary({
          id: editingMedia.id,
      ...
      showToast("Media record updated & replaced successfully!", "success")
        ...
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/75 p-4 backdrop-blur-sm">
          <form onSubmit={handleSaveEditMedia} className="w-full max-w-xl space-y-4 rounded-3xl bg-white p-6 text-slate-900 shadow-2xl">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              ...
              <button type="button" onClick={() => setEditingMedia(null)} aria-label="Close modal" className="p-1 rounded-full bg-slate-100 text-slate-500 hover:text-slate-900"><FiX size={18} /></button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <label className="block font-bold text-slate-800">Option 1: Upload Replacement Image File
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="font-bold text-slate-800">Caption
                  <input type="text" className="input-field mt-1" value={editForm.caption} onChange={(e) => setEditForm({ ...editForm, caption: e.target.value })} placeholder="Image description..." />
                </label>
                <label className="font-bold text-slate-800">Alt Text
                  <input type="text" className="input-field mt-1" value={editForm.alt_text} onChange={(e) => setEditForm({ ...editForm, alt_text: e.target.value })} placeholder="Accessible alt text..." />
                </label>
                <label className="font-bold text-slate-800">Photographer / Author
                  <input type="text" className="input-field mt-1" value={editForm.photographer} onChange={(e) => setEditForm({ ...editForm, photographer: e.target.value })} placeholder="Photographer name..." />
                </label>
                <label className="font-bold text-slate-800">License
                  <input type="text" className="input-field mt-1" value={editForm.license_type} onChange={(e) => setEditForm({ ...editForm, license_type: e.target.value })} placeholder="CC BY-SA 4.0 / Verified" />
                </label>
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
              <button type="button" onClick={() => setEditingMedia(null)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold text-xs">Cancel</button>
              <button type="submit" disabled={busy} className="px-5 py-2 rounded-xl bg-emerald-700 text-white font-black text-xs shadow">{busy ? "Saving..." : "Save Image Replacement"}</button>
            </div>
          </form>
        </div>
      )}
