...
export default function UserDashboardControlPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm space-y-4">
        <h3 className="font-extrabold text-lg text-slate-900 flex items-center gap-2">
          <FiZap className="text-purple-600" /> Interactive Engagement & AI Feature Controls
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div
            onClick={() => setEnableAIReplanning(!enableAIReplanning)}
            className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-center justify-between ${
              enableAIReplanning ? "bg-purple-50 border-purple-300" : "bg-slate-50 border-slate-200 opacity-60"
            }`}
          >
            <div>
            ...
            {enableAIReplanning ? <FiToggleRight size={24} className="text-purple-600" /> : <FiToggleLeft size={24} className="text-slate-400" />}
          </div>
