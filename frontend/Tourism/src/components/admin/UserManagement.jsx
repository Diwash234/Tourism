...
export default function UserManagement() {
  ...
  const anonymize = async () => {
    const confirmation = window.prompt(`Irreversible action. Type ${selected.email} to anonymize this account while retaining bookings and protected records.`)
    if (confirmation !== selected.email) return
  ...
  return <div className="space-y-5 text-slate-900">
    <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4">
      ...
      <button onClick={() => setCreating(true)} className="px-4 py-2.5 rounded-xl bg-emerald-600 text-white font-black text-sm flex items-center justify-center gap-2 whitespace-nowrap"><FiPlus/> Create account</button>
    </div>
    ...
    <div className="rounded-2xl border border-slate-700 bg-slate-900/70 overflow-hidden">
      ...
      <tbody className="divide-y divide-slate-800">{rows.map(u=><tr key={u.id} className="hover:bg-slate-800/50"><td className="p-4"><button onClick={()=>openDetail(u.id)} className="text-left"><b className="block text-white hover:text-amber-300">{u.full_name}</b><span className="text-xs text-slate-400">{u.email}</span><span className="block text-[10px] text-slate-500">{u.city || u.managed_district || "Location unavailable"}</span></button></td><td className="p-4"><select value={u.role} onChange={e=>mutate(u.id, ()=>adminApi.updateUserStatus(u.id,{role:e.target.value}), `Change this account's role to ${label(e.target.value)}?`)} className="bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs">{ROLES.map(r=><option key={r} value={r}>{label(r)}</option>)}</select></td><td className="p-4"><span className={u.is_verified?"text-emerald-300":"text-amber-300"}>{u.is_verified?"Verified":"Pending"}</span></td><td className="p-4 text-xs text-slate-400">{date(u.last_login)}</td><td className="p-4"><AdminStatusBadge value={u.is_active?"active":"inactive"}/></td><td className="p-4 text-right"><button onClick={()=>mutate(u.id,()=>adminApi.updateUserStatus(u.id,{is_active:!u.is_active}),`${u.is_active?"Deactivate":"Activate"} this account?`)} className="p-2 rounded-lg bg-slate-800" title="Toggle account access">{u.is_active?<FiUserX/>:<FiUserCheck/>}</button></td></tr>)}
      {!loading&&!rows.length&&<tr><td colSpan="6"><AdminEmptyState title="No accounts match these filters"/></td></tr>}</tbody></table></div>
      <div className="p-3 border-t border-slate-700"><AdminPagination page={page} pages={pages} onChange={setPage} disabled={loading}/></div>
    </div>
    ...
    {selected&&<div className="fixed inset-0 z-[70] bg-black/70 flex justify-end" onMouseDown={e=>e.target===e.currentTarget&&setSelected(null)}><aside className="w-full max-w-xl bg-slate-950 border-l border-slate-700 p-5 overflow-y-auto"><div className="flex justify-between"><div><h3 className="text-xl font-black">{selected.full_name||"User profile"}</h3><p className="text-sm text-slate-400">{selected.email}</p></div><button onClick={()=>setSelected(null)}><FiX size={22}/></button></div>{detailLoading?<p className="py-16 text-center text-slate-500">Loading profile…</p>:<div className="space-y-5 mt-6">
      ...
      <div className="grid sm:grid-cols-3 gap-2"><button onClick={()=>mutate(selected.id,()=>adminApi.runUserAccessAction(selected.id,selected.is_verified?"unverify":"verify"),"Update verified status?")} className="rounded-xl bg-emerald-700 px-3 py-2 text-sm font-bold flex justify-center gap-2"><FiCheckCircle/> {selected.is_verified?"Remove verification":"Mark verified"}</button><button onClick={()=>mutate(selected.id,()=>adminApi.runUserAccessAction(selected.id,"revoke_sessions"),"Revoke all recorded refresh sessions for this user?")} className="rounded-xl bg-rose-800 px-3 py-2 text-sm font-bold flex justify-center gap-2"><FiKey/> Revoke sessions</button><button onClick={anonymize} className="rounded-xl bg-rose-950 border border-rose-700 px-3 py-2 text-sm font-bold">Anonymize</button></div>
