...
export default function Emergency() {
  ...
  return (
    <div className="ny-page container-app space-y-6 py-6 sm:py-8" data-testid="emergency-page">
      ...
      {loading ? <SkeletonLoader count={3} type="card" /> : !directory ? <EmptyState title="Choose a place to see local help" subtitle="Search a destination, use your location, or pick a city below. We never guess a default city or invent facilities." action={<div className="flex flex-wrap justify-center gap-2"><button type="button" onClick={() => position ? loadCoordinates(position.lat, position.lng) : requestLocation()} disabled={locating} className="ny-btn ny-btn-primary"><FiMapPin size={15} aria-hidden="true" />{locating ? "Finding location…" : "Use my location"}</button>{QUICK_PLACES.map((place) => <button key={place} type="button" onClick={() => { setQuery(place); loadDestination(place) }} className="ny-btn ny-btn-secondary">{place}</button>)}</div>} /> : <>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-[var(--ny-text-muted)]">Emergency coverage around</p><h2 className="mt-1">{locationTitle}</h2><p className="mt-1 text-sm text-[var(--ny-text-secondary)]">{directory.location?.district || "Location unavailable"}{directory.location?.province ? ` · ${directory.location.province}` : ""} · {directory.radius_km || radius} km radius</p></div>{(directory.coverage_gap || (counts.hospitals_within_radius === 0 && counts.police_within_radius === 0)) && <p className="max-w-md rounded-[var(--ny-radius-md)] border border-[#E9D39A] bg-[var(--ny-soft-gold)] p-3 text-sm text-[var(--ny-warning)]">No hospital or police record is available for this area. <Link to="/submit-service" className="font-semibold underline">Submit a facility for review.</Link></p>}</div>
